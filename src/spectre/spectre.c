...
byte* initialize() {
    const unsigned secret_table_size = 4 << 12;
...
byte speculativelyReadAddress(unsigned address, unsigned page_size, unsigned probe_length) {
    byte* valid_access_indicator_array = (byte*) malloc(TRAINING_CONSTANT * page_size);
    ...
    training_counter = 0;
    for (unsigned i = 0; i < TRAINING_CONSTANT; ++i) {
        ...
        const unsigned fixed_page_size = invalid_access * page_size;
        // load condition from RAM so speculative execution gets triggered
